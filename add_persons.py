...
# Check if CUDA is available and set the device accordingly  == GPU hai to us pr kam varan CPU pr kam krna
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Initialize the face detector (Kisi ek detectors ko use kiya jo model hai unme se)
...
# Face image → AI model → numerical features (embedding)
@torch.no_grad()
def get_feature(face_image):
    ...
    """
    #  image preprocessing steps ye image ko model ke liye prepare krne ke liye kiya jata hai taki model ko sahi input mile aur achhe features extract ho sake
